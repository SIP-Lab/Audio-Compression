...
- (void)predict: (VADNoiseReductionCompression *) vadNoiseRedCom  settings: (Settings *) settings{
    ...
    //settings->classLabel = round(pred(0,1)) + 1; // in prev. version of the app
    settings->classLabel = (settings->dbpower < settings->quiet) ? 0 : round(pred(0,1)) + 1;// in personalized app
    NSLog(@"%f", pred(0,1));
}
